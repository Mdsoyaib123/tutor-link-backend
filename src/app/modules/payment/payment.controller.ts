...
const order = await Payment.findOne({ transaction:transactionId });
        console.log(order)
await PermitTutor.findByIdAndUpdate(order?.requestId, { isPayment: true }, { new: true });

        
    return res.redirect(`/success/${transactionId}`); //here will add frountend base url or vercel url
};
export const failPayment = async (req: Request, res: Response) => {
    ...
    return res.redirect(`/fail`); //here will add frountend base url or vercel link
};
...
export const cancelPayment = async (req: Request, res: Response) => {
    ...
    return res.redirect(`/cancel`); //here will add frountend base url or vercel link
};
